...
function stampComposedIds(body, slideId) {
  const classIdMap = new Map();
  return body.replace(/(<(?:div|section|article|aside|header|footer|main)\b[^>]*\sclass="([^"]+)"[^>]*)(>)/g, (match, open, cls, close) => {
    if (open.includes('data-feedback=') || /\sid="/.test(open)) return match;
    const firstClass = cls.trim().split(/\s+/)[0];
    const count = (classIdMap.get(firstClass) || 0) + 1;
    classIdMap.set(firstClass, count);
    const suffix = count > 1 ? `-${count}` : '';
    return `${open} id="${slideId}--${firstClass}${suffix}" data-feedback="true"${close}`;
  });
}
...
export function buildSlide(projectDir, slideId, page = null) {
  ...
  if (slide.template === 'composed') {
    const rawBody = slide.html_body || `<div class="slide" style="display:grid;place-items:center"><p style="color:var(--p700);font-size:24px">Empty composed slide: ${slideId}</p></div>`;
    const body = stampComposedIds(rawBody, slideId);
