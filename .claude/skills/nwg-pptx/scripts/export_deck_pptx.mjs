...
async function renderComposedAsImage(pptx, slideJson, slideHtmlPath, page) {
  ...
  try {
    ...
    try { puppeteer = (await import(pathToFileURL('/opt/homebrew/lib/node_modules/puppeteer/index.js').href)).default; } catch {}
  }
  if (!puppeteer) {
  ...
  const imgBuf = await pg.screenshot({ type: 'png' });
  await browser.close();
  const imgB64 = imgBuf.toString('base64');
  slide.addImage({ data: `image/png;base64,${imgB64}`, x: 0, y: 0, w: W, h: H });
